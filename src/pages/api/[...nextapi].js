...
async function handleAddCustomer(req, res) {
  const { fullname, emailaddress, address, contactNumber } = req.body;
  try {
    const [result] = await db.query(
      'INSERT INTO acustomer (fullname, emailaddress, address, contactNumber) VALUES (?, ?, ?, ?)',
      [fullname, emailaddress, address, contactNumber]
    );
    res.status(201).json({ id: result.insertId, fullname, emailaddress, address, contactNumber });
    console.log('Customer added successfully:', { id: result.insertId, fullname, emailaddress, address, contactNumber });
  } catch (error) {
...
async function handleUpdateCustomer(req, res, id) {
  ...
  try {
    await db.query(
      'UPDATE acustomer SET fullname = ?, emailaddress = ?, address = ?, contactNumber = ? WHERE customerid = ?',
      [fullname, emailaddress, address, contactNumber, id]
    );
    res.status(200).json({ id, fullname, emailaddress, address, contactNumber });
    console.log('Customer updated successfully:', { id, fullname, emailaddress, address, contactNumber });
  } catch (error) {
